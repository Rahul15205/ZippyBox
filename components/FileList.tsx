...
export default function FileList({
  ...
  const filteredFiles = useMemo(() => {
    console.log("Filtering files. Total files:", files.length, "Active tab:", activeTab);
    const filtered = (() => {
      switch (activeTab) {
        case "starred":
  ...
  const previewFile = (file: FileType) => {
    if (file.type.startsWith("image/")) {
      // For images, open in optimized viewer
      const optimizedUrl = `${process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}/tr:q-90,w-1600,h-1200,fo-auto/${file.path}`;
      window.open(optimizedUrl, "_blank");
      ...
      // For other files, show a message that preview is not available
      addToast({
        title: "Preview Not Available",
        description: `Preview is not available for ${file.type} files. Please download to view.`,
        color: "warning",
      });
    }
  };
